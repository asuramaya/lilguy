...
- **Ashby**: URL contains `jobs.ashbyhq.com/<token>`.
- **SmartRecruiters**: URL contains `jobs.smartrecruiters.com/<token>/...`.
  `<token>` is case-sensitive, usually the brand as written ("Bosch"),
  not a lowercased slug.
- **Rippling**: URL contains `ats.rippling.com/<token>/jobs`.
- **Workable**: URL contains `apply.workable.com/<token>/`.
- **BambooHR**: URL contains `<token>.bamboohr.com/careers`. `<token>`
  is the subdomain.
- **Taleo**: URL contains `<tenant>.taleo.net/careersection/<section>/
  jobsearch.ftl`. Both `<tenant>` and `<section>` come straight off the
  URL. Some larger Taleo deployments are hosted at an opaque
  `fa###.taleo.net` host instead — not usable here, same shape as Oracle
